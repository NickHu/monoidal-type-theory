...
repository's multicategory development.  The prototype is
`Multicategory/Free.agda` (syntax, substitution, β/η-congruence — all
typechecked); this note records the design mapping, the findings, and the
roadmap to Theorem 2.4.10.
...
| Theorem 2.4.6/2.4.10 multicategory | roadmap items 1–2 below |
| representability | roadmap item 3; β⊗/η⊗ are literally the two round-trips of the universality equivalence at `⦅var,var⦆` |
...
5. **Infrastructure fit.**  `sub`'s type is exactly `_∘ₘ_`; `sub`'s `var`
   ...
   precisely `Premulticategory` + `is-representable`, so once roadmap items
   1–3 land, the *free strict monoidal category* on a multigraph is obtained
   by composition with existing code.
...
3. **Representability.**  `⦅var,var⦆ : Homₘ (A ∷ B ∷ []) (A ⊗ B)` is
   ...
   hence via `Strictification` the free strict monoidal category on G.
...
4. **Freeness (Theorem 2.4.10).**  For `M` representable and `P : G → M` a
   ...
   notion of multifunctor, which the repo does not yet have.
